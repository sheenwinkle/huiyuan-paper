...
<body>
  ...
  <main id="inquiry" class="view hidden"><section><div class="shell grid grid2"><div><h2>在线咨询</h2><p>留下产品、数量、地区和联系方式。价格、库存、发货、账期由人工确认。</p><div class="card">地址：江苏省镇江市丹阳市丹北镇埤城镇</div></div><form id="inquiryForm" class="form panel"><input name="name" required placeholder="姓名"><input name="phone" required placeholder="手机"><input name="wechat" placeholder="微信"><input name="region" placeholder="所在地区"><select name="product" required></select><select name="customerType"><option value="UNKNOWN">客户类型：暂不确定</option><option value="WHOLESALER">批发商</option><option value="RETAILER">零售商</option></select><input name="quantity" placeholder="预计数量"><textarea name="note" rows="4" placeholder="需求说明"></textarea><button class="btn primary">提交询盘</button><p id="inquiryStatus"></p></form></div></section></main>
  ...
  <script>
    ...
    function renderProducts(target){$(target).innerHTML=cards().map(p=>'<div class="card">'+(p.imageUrl?'<img src="'+p.imageUrl+'" alt="'+p.name+'" style="width:100%;aspect-ratio:4/3;object-fit:cover;border-radius:6px;margin-bottom:14px">':'<div class="icon">'+p.name.slice(0,1)+'</div>')+(p.category?'<small style="color:var(--red);font-weight:700">'+p.category+'</small>':'')+'<h3>'+p.name+'</h3><p>'+p.description+'</p></div>').join("");}
    ...
    $("#inquiryForm").onsubmit=e=>{e.preventDefault();const f=new FormData(e.target);state.inquiries.unshift({id:crypto.randomUUID(),createdAt:new Date().toLocaleString(),status:"待跟进",priority:"普通",customerType:f.get("customerType")||"UNKNOWN",followUpNote:"",nextFollowUpAt:"",name:f.get("name"),phone:f.get("phone"),wechat:f.get("wechat"),region:f.get("region"),product:f.get("product"),quantity:f.get("quantity"),note:f.get("note")});save();e.target.reset();$("#inquiryStatus").textContent="已收到询盘，后台可查看。";};
    ...
    function renderAdmin(tab){const box=$("#adminContent"); if(tab==="products"){box.innerHTML='<div class="grid grid2"><form id="catForm" class="form panel"><h3>新增分类</h3><input name="name" required placeholder="分类名"><textarea name="description" required placeholder="说明"></textarea><button class="btn primary">保存分类</button></form><form id="prodForm" class="form panel"><h3>新增产品</h3><select name="categoryId">'+state.categories.map(c=>'<option value="'+c.id+'">'+c.name+'</option>').join("")+'</select><input name="name" required placeholder="产品名"><input name="imageUrl" placeholder="产品图片链接"><textarea name="description" required placeholder="说明"></textarea><button class="btn primary">保存产品</button></form></div><div class="grid" style="margin-top:18px">'+state.products.map(p=>'<div class="card">'+(p.imageUrl?'<img src="'+p.imageUrl+'" alt="'+p.name+'" style="width:100%;max-height:160px;object-fit:cover;border-radius:6px;margin-bottom:12px">':'')+'<strong>'+p.name+'</strong><p>'+p.description+'</p><button class="btn secondary" onclick="toggleProduct(\''+p.id+'\')">'+(p.active?'下架':'上架')+'</button></div>').join("")+'</div>';$("#catForm").onsubmit=e=>{e.preventDefault();const f=new FormData(e.target);state.categories.push({id:crypto.randomUUID(),name:f.get("name"),description:f.get("description"),active:true});save();renderAdmin("products");render();};$("#prodForm").onsubmit=e=>{e.preventDefault();const f=new FormData(e.target);state.products.push({id:crypto.randomUUID(),categoryId:f.get("categoryId"),name:f.get("name"),imageUrl:f.get("imageUrl"),description:f.get("description"),active:true});save();renderAdmin("products");render();};}
      if(tab==="inquiries"){const due=state.inquiries.filter(i=>i.nextFollowUpAt&&new Date(i.nextFollowUpAt)<=new Date()).length;box.innerHTML='<div class="grid grid3"><div class="card">总线索：<strong>'+state.inquiries.length+'</strong></div><div class="card">高优先级：<strong>'+state.inquiries.filter(i=>i.priority==="高").length+'</strong></div><div class="card">今日/超期：<strong>'+due+'</strong></div></div><div class="adminbar"><button class="btn secondary" onclick="exportInquiries()">导出 CSV</button></div><table class="table"><tr><th>时间</th><th>客户</th><th>电话</th><th>类型</th><th>产品</th><th>优先级</th><th>下次跟进</th><th>跟进记录</th><th>状态</th></tr>'+state.inquiries.map(i=>'<tr><td>'+i.createdAt+'</td><td>'+i.name+'</td><td>'+i.phone+'</td><td><select onchange="setInquiryField(\''+i.id+'\',\'customerType\',this.value)"><option value="UNKNOWN" '+((i.customerType||"UNKNOWN")==="UNKNOWN"?"selected":"")+'>未确认</option><option value="WHOLESALER" '+(i.customerType==="WHOLESALER"?"selected":"")+'>批发商</option><option value="RETAILER" '+(i.customerType==="RETAILER"?"selected":"")+'>零售商</option></select></td><td>'+i.product+'</td><td><select onchange="setInquiryField(\''+i.id+'\',\'priority\',this.value)"><option '+((i.priority||"普通")==="低"?"selected":"")+'>低</option><option '+((i.priority||"普通")==="普通"?"selected":"")+'>普通</option><option '+(i.priority==="高"?"selected":"")+'>高</option></select></td><td><input type="date" value="'+(i.nextFollowUpAt||"")+'" onchange="setInquiryField(\''+i.id+'\',\'nextFollowUpAt\',this.value)"></td><td><textarea rows="2" onchange="setInquiryField(\''+i.id+'\',\'followUpNote\',this.value)">'+(i.followUpNote||"")+'</textarea></td><td><select onchange="setInquiryField(\''+i.id+'\',\'status\',this.value)"><option '+(i.status==="待跟进"?"selected":"")+'>待跟进</option><option '+(i.status==="已联系"?"selected":"")+'>已联系</option><option '+(i.status==="已关闭"?"selected":"")+'>已关闭</option></select></td></tr>').join("")+'</table>';}
    ...
    window.toggleProduct=id=>{const p=state.products.find(x=>x.id===id);p.active=!p.active;save();renderAdmin("products");render();}; window.setInquiryField=(id,key,value)=>{const i=state.inquiries.find(x=>x.id===id);i[key]=value;save();renderAdmin("inquiries");}; window.exportInquiries=()=>{const rows=[["时间","客户","电话","微信","类型","产品","数量","地区","状态","优先级","下次跟进","需求备注","跟进记录"],...state.inquiries.map(i=>[i.createdAt,i.name,i.phone,i.wechat,i.customerType,i.product,i.quantity,i.region,i.status,i.priority,i.nextFollowUpAt,i.note,i.followUpNote])];const csv="\ufeff"+rows.map(r=>r.map(v=>'"'+String(v||"").replaceAll('"','""')+'"').join(",")).join("\n");const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([csv],{type:"text/csv;charset=utf-8"}));a.download="huiyuan-inquiries.csv";a.click();};
